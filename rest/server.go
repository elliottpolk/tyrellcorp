...
func Serve(ctx context.Context, comp *config.Composition) error {
	...
	// register services
	if err := tyrellcorp.RegisterSpecServiceHandlerFromEndpoint(_ctx, mux, fmt.Sprintf(":%s", comp.Server.RpcPort), opts); err != nil {
		return errors.Wrap(err, "unable to register spec service handler")
	}
